...
*/
	...
	public boolean checkAdminLogin(String user, String pass) {
		...
		try {
			s = c.createStatement();
			s.setQueryTimeout(3);
			s.execute("select login,pass from admins");
			ResultSet r = s.getResultSet();
			r.first();
			do {
				if(user.equals(r.getString(1))) {
	...
	public String[] retrieveEmployees(){
		...
		try {
			...
			r.first();
			do {
				sbu.append(r.getString(1));
				sbp.append(r.getString(2));
				sbu.append("\n");
				sbp.append("\n");
			}
			while(r.next());
		...
		String[] retval = {sbu.toString(),sbp.toString()};
		return retval;
	}
	
	public String[] retrieveAdmins(){
	...
	public void makeNewAdminList(String[] us, String[] ps) {
		Statement s;
		try {
			s = c.createStatement();
			s.setQueryTimeout(3);
			if(us.length>0){
				s.execute("delete from admins");
				for(int i=0;i<us.length;i++){
					s.execute("insert into admins (login,pass) values(\""+us[i]+"\",\""+ps[i]+"\")");	
				}
			}
			s.close();
		} catch (SQLException e) {
	...
	public void makeNewEmployeeList(String[] us, String[] ps){
		Statement s;
		try {
			s = c.createStatement();
			s.setQueryTimeout(3);
			if(us.length>0){
				...
				for(int i=0;i<us.length;i++){
					s.execute("insert into employees (login,pass) values(\""+us[i]+"\",\""+ps[i]+"\")");	
				}
			}
			s.close();
		} catch (SQLException e) {
